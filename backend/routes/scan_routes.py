...
def run_scan():
    ...
    try:
        ...
        return jsonify({"message": "Scan terminé", "scan": scan.to_dict()}), 200
...
# ============================================
# GET /api/scans/modules/actifs
# ============================================
@scan_bp.route("/modules/actifs", methods=["GET"])
@jwt_required()
def get_modules_actifs():
    modules = Module.query.filter_by(is_active=True).all()
    return jsonify({"modules": [m.key for m in modules]}), 200
...
def get_historique():
    ...
    scans = Scan.query.filter_by(user_id=user_id)\
                      .order_by(Scan.date.desc()).all()
    return jsonify({"scans": [s.to_dict() for s in scans], "total": len(scans)}), 200
...
def get_par_module(module):
    ...
    vulns = db.session.query(Vulnerability)\
                      .join(Scan)\
                      .filter(Scan.user_id == user_id, Vulnerability.module == module)\
                      .order_by(Scan.date.desc()).all()
    return jsonify({"module": module, "vulnerabilites": [v.to_dict() for v in vulns], "total": len(vulns)}), 200